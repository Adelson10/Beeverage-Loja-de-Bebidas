...
import { ArrowsClockwise, CaretDown, CaretUp, Chat, ChatCircleText, House, Motorcycle, Package, Storefront } from '@phosphor-icons/react';
...
const Pedidos = () => {
    ...
    const [expandedIds, setExpandedIds] = useState<number[]>(PedidosMockup[0] ? [PedidosMockup[0].id] : []);
    ...
    if (!pedidosProdutos.length) return (
        <div className="flex flex-col items-center justify-center gap-4 py-[4rem] text-center">
            ...
            <a href="/" className="py-3 px-6 rounded-lg text-white font-normal bg-linear-to-r from-brand to-brand-dark hover:bg-brand-dark">Ver Catálogo</a>
        </div>
    )
        ...
        <div className="flex gap-4 items-start flex-col">
            ...
            <div className="flex flex-col gap-4 w-full">
                { pedidosProdutos.map(({ pedido, produtos }) => {
                    ...
                    return (
                    <div>
                        <div key={pedido.id} className="flex flex-col gap-4 p-4 bg-white/40 rounded-[16px] max-[600px]:p-3">
                            <div className="flex items-center gap-4 max-[600px]:gap-2">
                                <div className="shrink-0 hidden min-[601px]:flex items-center justify-center">
                                    <div className="relative">
                                        <PhotoProduct color1="#FFFFFF" color2="#CECECE" shadowImage={primary.thumbnail.shadowWidth} srcImg={primary.thumbnail.src} />
                                ...
                                <div className="shrink-0 flex min-[601px]:hidden items-center justify-center">
                                    <div className="relative">
                                        <PhotoProduct type="Carrinho" color1="#FFFFFF" color2="#CECECE" shadowImage={primary.thumbnail.shadowWidth} srcImg={primary.thumbnail.src} />
                                        {primary.price !== 0 && <div style={{ backgroundImage: `url(${discountIcon})` }} className='absolute bottom-0 h-[35px] w-[35px] bg-center flex items-center justify-center text-white text-[0.6rem]'>{(((primary.priceNow * 100) / primary.price) - 100).toFixed(0)}%</div>}
                                    </div>
                                </div>
                                <div className="flex flex-col flex-1 gap-[1rem]">
                                    <div className="flex justify-between items-start">
                                        <div className="flex-1 flex flex-col gap-[0.5rem]">
                                            ...
                                            <p className="text-primary text-[1rem]"><strong className="font-medium">Volume:</strong> {primary.volume}</p>
                                            <p className="text-primary text-[1rem]">Comprado em {pedido.data} | {totalQuantidade} Unidades | <strong className="text-brand-dark font-medium">{subtotal.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}</strong></p>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between gap-4 max-[600px]:gap-2">
                                        <button onClick={() => handleBuyAgain(produtos)} className="flex items-center gap-[0.5rem] w-fit px-[20px] py-[12px] rounded-lg text-white font-normal text-[0.9rem] bg-linear-to-r from-brand to-brand-dark hover:bg-brand-dark">
                                            <ArrowsClockwise size={16} weight="bold" />Comprar Novamente
                                        </button>
                                        <button type="button" onClick={() => toggleExpand(pedido.id)} className="flex items-center gap-1 self-end text-brand-dark text-[1rem] border-none bg-transparent cursor-pointer hover:text-brand-select">
                                            Ver Informações 
                        ...
                        { isExpanded &&
                                <div className="flex flex-col gap-[1rem] p-[1rem]">
                                    <div className="grid grid-cols-3 gap-4 max-[750px]:grid-cols-1 text-[0.85rem]">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <h4 className="font-semibold text-brand-dark mb-1">Resumo do Pedido</h4>
                                            <p className="flex items-center justify-between text-secundary"><span>Valor Unitário:</span><strong className="font-medium">{primary.priceNow.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}</strong></p>
                                            <p className="flex items-center justify-between text-secundary"><span>Quantidade:</span><strong className="font-medium">{totalQuantidade}</strong></p>
                                            <p className="flex items-center justify-between text-secundary"><span>Subtotal:</span><strong className="font-medium">{subtotal.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}</strong></p>
                                            <p className="flex items-center justify-between text-secundary border-t border-primary/30 pt-1"><span>Frete:</span><strong className="font-medium">{pedido.frete.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}</strong></p>
                                            <p className="flex items-center justify-between border-t border-primary/30 pt-1"><span className="font-medium text-secundary">Total:</span><strong className="text-brand-dark text-[1.1rem]">{total.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}</strong></p>
                                        </div>
                                    </div>

                                    { step >= 0 &&
                                        <div className="flex items-center gap-2 px-2">
                                            <div className={`flex items-center justify-center h-8 w-8 rounded-lg shrink-0 ${step >= 0 ? 'bg-brand-dark text-white' : 'bg-white text-primary border border-primary'}`}>
                                                <Storefront size={16} weight="fill" />
                                            </div>
                                            <div className={`flex-1 h-1 rounded-full ${step >= 1 ? 'bg-brand-dark' : 'bg-primary/30'}`}></div>
                                            <div className={`flex items-center justify-center h-8 w-8 rounded-lg shrink-0 ${step >= 1 ? 'bg-brand-dark text-white' : 'bg-white text-primary border border-primary'}`}>
                                                <Motorcycle size={16} weight="fill" />
                                            </div>
                                            <div className={`flex-1 h-1 rounded-full ${step >= 2 ? 'bg-brand-dark' : 'bg-primary/30'}`}></div>
                                            <div className={`flex items-center justify-center h-8 w-8 rounded-lg shrink-0 ${step >= 2 ? 'bg-brand-dark text-white' : 'bg-white text-primary border border-primary'}`}>
